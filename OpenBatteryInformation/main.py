...
class OBI(tk.Tk):
    def __init__(self):
        ...
        self.module_combobox.pack(pady=10)
        ...
        self.interface_combobox = ttk.Combobox(interface_frame, textvariable=self.interface_var, width=20)
    ...
    def display_module(self, event=None):
        ...
        if selected_module and selected_interface:
            try:
                ...
                self.clear_main_window()
                main_app = module_to_display.ModuleApplication(self.main_window, interface_module.Interface)

                main_app.set_interface(self.current_interface)
            except ImportError as e:
                print(f"Error loading module or interface: {e}")
